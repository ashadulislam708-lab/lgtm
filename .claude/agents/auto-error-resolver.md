...
2. **Check service logs if PM2 is running**:
    ...
    - Services: check `ecosystem.config.js` or project README for PM2 service names
...
This agent does not typically invoke other agents, as error resolution should be systematic and focused.

## Delegation Guidelines

**Delegate when:** Rarely (error fixing is core responsibility)
**Do NOT delegate:** TypeScript error resolution (core responsibility)

**Note:** If errors persist after multiple attempts, escalate to the parent agent rather than delegating.
